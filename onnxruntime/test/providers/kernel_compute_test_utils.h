...
class KernelComputeTester {
 public:
  ...
  void AddInput(const char* name, std::initializer_list<int64_t> dims, std::initializer_list<T> values,
    ...
    AddData(input_data_, name, std::vector<int64_t>(dims), values.begin(), std::vector<int64_t>(strides), is_cpu_data);
  }

  template <typename T>
  void AddOutput(const char* name, std::initializer_list<int64_t> dims, std::initializer_list<T> values,
                 std::initializer_list<int64_t> strides = {}) {
    AddData(output_data_, name, std::vector<int64_t>(dims), values.begin(), std::vector<int64_t>(strides), false);
  }

  template <typename T>
  void AddAttribute(std::string name, T value) {
    // Generate a the proper AddAttribute call for later.
    add_attribute_funcs_.emplace_back([name = std::move(name), value = std::move(value)](Node& node) {
 ...
 private:
  template <typename T>
  void AddData(std::vector<Data>& data, const char* name, std::vector<int64_t> dims, const T* values,
               std::vector<int64_t> strides, bool is_cpu_data) {
    OrtValue value;

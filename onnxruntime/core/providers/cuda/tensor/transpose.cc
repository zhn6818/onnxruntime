...
Status Transpose::DoTranspose(const cudaDeviceProp& prop,
  ...
  const auto input_size = input_shape_override ? input_shape_override->Size() : input.Shape().Size();
  ...
  if (new_rank <= 1) {
    CUDA_RETURN_IF_ERROR(cudaMemcpyAsync(output.MutableDataRaw(), input.DataRaw(),
                                         static_cast<size_t>(input_size) * input.DataType()->Size(), cudaMemcpyDeviceToDevice,
                                         stream));
    return Status::OK();
  }
  ...
  if (is_contiguous && (element_type == ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT ||
  ...
  if (CanDoTranspose3D(prop, static_cast<size_t>(new_rank), new_input_dims, new_permutations, grid_size, block_size)) {
    return Transpose3DImpl(stream, element_size, ToConstSpan(new_input_strides), ToConstSpan(new_output_strides),
  ...
  // Transpose4DParallelizeMultipleElementsPerThreadInInnermostDim doesn't support strided tensor.
  if (is_contiguous) {
    if (new_rank == 3) {
      new_permutations[0]++;
      new_permutations[1]++;
      ...
      new_input_strides.insert(new_input_strides.begin(), input_size);
      new_output_strides.insert(new_output_strides.begin(), input_size);
      new_rank = 4;
    }
    ...
    if (CanDoTranspose4DParallelizeMultipleElementsPerThreadInInnermostDim(
      ...
      return Transpose4DParallelizeMultipleElementsPerThreadInInnermostDim(
          stream, element_size, ToConstSpan(new_input_dims), ToConstSpan(new_input_strides), input.DataRaw(),
...
Status Transpose::ComputeInternal(OpKernelContext* ctx) const {
  ...
  if (p_X == nullptr) return Status(common::ONNXRUNTIME, common::FAIL, "input count mismatch");
  const Tensor& X = *p_X;

...
#include "layers/beam_search_layers/BeamSearchLayer.h"
#include "layers/beam_search_layers/OnlineBeamSearchLayer.h"
//#include "layers/sampling_layers/TopKSamplingLayer.h"
//#include "layers/sampling_layers/TopKTopPSamplingLayer.h"
                          ...
                          const std::vector<Tensor>* input_tensors)
{
    ...
    for (int step = 1; step <= (int)max_seq_len_; step++) {
        check_cuda_error(cudaMemcpy(h_finished_buf_, finished_buf_, sizeof(bool) * batch_size * beam_width_, cudaMemcpyDeviceToHost));
    ...
    invokeMinusUnfinishedSeqlen((int*)output_tensors->at(2).data, finished_buf_, gsl::narrow_cast<int>(batch_size * beam_width_), stream_);
    ...
    if (beam_width_ > 1) {
        ...
        check_cuda_error(cudaMemcpy((int*)output_tensors->at(1).data,
                                    parent_ids_buf_ + batch_size * beam_width_,
                                    sizeof(int) * batch_size * beam_width_ * (max_seq_len_ - 1),
                                    cudaMemcpyDeviceToDevice));

        invokeGatherTree((int*)output_tensors->at(0).data,
                         (int*)output_tensors->at(2).data,
                         gsl::narrow_cast<int>(max_seq_len_ - 1),
                         gsl::narrow_cast<int>(batch_size),
                         gsl::narrow_cast<int>(beam_width_),

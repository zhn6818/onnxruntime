...
#include "cub/cub.cuh"
#include "beam_search_topk_kernels.h"
...
//                                    const unsigned int d_model)
// {
//     if (blockIdx.x == 0) {
//         for (unsigned int i = threadIdx.x; i < batch_size * beam_width; i += blockDim.x) {
...
//     for (unsigned int i = threadIdx.x; i < d_model; i += blockDim.x) {

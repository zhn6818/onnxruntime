                              ...
                              cudaStream_t stream)
{
    ...
    dim3 grid(CeilDiv(batch_size * beam_width, 256));
    dim3 block(256);
                            ...
                            cudaStream_t stream)
{
    ...
    dim3 grid(CeilDiv(hidden_unit * vocab_size_padded, 512));
                      ...
                      cudaStream_t stream)
{
    ...
    if (batchbeam > 1024) {
        grid.x = CeilDiv(batch_size * beam_width, 1024);
        block.x = 1024;
    }
...
void invokeMinusUnfinishedSeqlen(int* sequence_lengths, const bool* finished, const int token_num, cudaStream_t stream)
{
    ...
    dim3 grid(CeilDiv(token_num, 256));

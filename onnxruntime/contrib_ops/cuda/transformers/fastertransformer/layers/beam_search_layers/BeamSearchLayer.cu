 ...
 */
#include "gsl/gsl"
#include "contrib_ops/cuda/transformers/fastertransformer/kernels/beam_search_topk_kernels.h"
#include "contrib_ops/cuda/transformers/fastertransformer/kernels/reduce_kernel_utils.cuh"
...
void BeamSearchLayer<T>::invokeSoftMax(std::vector<Tensor>* output_tensors, const std::vector<Tensor>* input_tensors)
{
    ...
    const int id_offset = (step * batch_size + ite * local_batch_size) * gsl::narrow_cast<int>(beam_width_);
    invokeLogProbAddCumLogProb(float_log_prob_buf_,
                               ...
                               local_batch_size * gsl::narrow_cast<int>(beam_width_),
...
void BeamSearchLayer<T>::allocateBuffer()
{
    if (is_allocate_buffer_ == false) {
        invokeTopkBeamSearch<float>(nullptr,
                                    ...
                                    nullptr,
                                    nullptr,
                                    gsl::narrow_cast<int>(max_batch_size_),
                                    gsl::narrow_cast<int>(beam_width_),
                                    gsl::narrow_cast<int>(vocab_size_padded_),
                                    0.0f,
                                    end_id_,

...
#include "contrib_ops/cuda/transformers/fastertransformer/kernels/beam_search_penalty_kernels.h"
#include "contrib_ops/cuda/transformers/fastertransformer/utils/cuda_utils.h"
...
bool BaseBeamSearchLayer<T>::isValidBatchSize(size_t batch_size)
{
    if (batch_size <= max_batch_size_) {
        ...
        freeBuffer();
        max_batch_size_ = static_cast<size_t>(batch_size * 1.2);
        return true;
    }
}
...
void BaseBeamSearchLayer<T>::forward(std::vector<Tensor>* output_tensors, const std::vector<Tensor>* input_tensors)
{
    ...
    const int batch_size = gsl::narrow_cast<int>(output_tensors->at(0).shape[1]);
    const int step = *((int*)input_tensors->at(2).data);
    const int ite = *((int*)input_tensors->at(7).data);
    const int local_batch_size = gsl::narrow_cast<int>(input_tensors->at(0).shape[0]);
    ...
    if (input_tensors->at(1).data != nullptr || temperature_ != 1.0f || len_penalty_ != 1.0f
        ...
        invokeAddBiasApplyPenalties(step,
                                    ...
                                    *(int*)input_tensors->at(5).data,
                                    local_batch_size,
                                    batch_size,
                                    gsl::narrow_cast<int>(beam_width_),
                                    gsl::narrow_cast<int>(vocab_size_),
                                    gsl::narrow_cast<int>(vocab_size_padded_),
                                    end_id_,
                                    temperature_,
                                    len_penalty_,
                                    repetition_penalty_,
    ...
    if (beam_width_ > 1) {
        const int max_seq_len = gsl::narrow_cast<int>(output_tensors->at(0).shape[0]);
        const int decoder_layer = gsl::narrow_cast<int>(output_tensors->at(5).shape[0]);
        const int decoder_max_seq_len = gsl::narrow_cast<int>(output_tensors->at(5).shape[4]);
        FT_CHECK(max_seq_len == decoder_max_seq_len);
        int cache_size = 1;
        for (auto t = output_tensors->at(5).shape.begin() + 1; t != output_tensors->at(5).shape.end(); ++t) {
            cache_size *= gsl::narrow_cast<int>(*t);
        }
        ...
        if (output_tensors->at(5).type == DataType::TYPE_FP32) {
            update_KV_cache_kernelLauncher((float*)output_tensors->at(5).data,
                                           ...
                                           local_batch_size,
                                           gsl::narrow_cast<int>(beam_width_),
                                           gsl::narrow_cast<int>(head_num_),
                                           gsl::narrow_cast<int>(size_per_head_),
                                           step,
                                           decoder_max_seq_len,
                                           cache_size,
                                           decoder_layer,
